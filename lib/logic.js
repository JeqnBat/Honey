...
const findNames = (dayIndex, data) => {
  ...
  while (Boolean(cell[`A${cellNb+i}`] && i < 20)) {
    if (cell[`A${cellNb+i}`].v !== undefined && typeof cell[`A${cellNb+i}`].v !== 'number' && !cell[`A${cellNb+i}`].v.includes('Effectif')) {
      const employee = {
  ...
  const weekOverview = weeklyStats(data)
  employees.forEach((el) => {
    // prevent bug when last element's key 'name' is deleted => !== undefined ?
    if (weekOverview.find((element) => element.name.trim() === undefined)) {
      console.log('undefined')
    } else {
      el.weeklyStats = weekOverview.find((element) => element.name.trim().toLowerCase() === el.name.trim().toLowerCase())
      delete weekOverview.name
    }
...
// 8. FINDWEEKLYSSTATS ____________________________________ */
/**
 * <b>DESCR:</b><br>
 * Parse small table at the end of the week sheet & extracts
 * data concerning global week stats for each employee
 * 
 * @param { object } data JS object representing all week tables
 * @return { object } 4 entries to be added to the main 'employee' object
 * 
 */
const weeklyStats = (data) => {
  let dayIndex, indivStats
  let stats = []
  let i = 1
  readCol('A', data).map((el) => {
    let str = data[el].w
    if (typeof str === 'string' && str.includes('Nom')) {
      dayIndex = Number(el.slice(1))
    }
  })
  while (data[`A${dayIndex + i}`] !== undefined) {
    if (i === 20) {
      break
    }
    let firstStats = {
      name: data[`A${dayIndex + i}`].v,
      forecastHrs: data[`B${dayIndex + i}`].v,
      actualHrs: data[`E${dayIndex + i}`].v,
      difference: data[`G${dayIndex + i}`].v
    }
    if (data[`I${dayIndex + i}`] !== undefined) {
      const capiNote = data[`I${dayIndex + i}`].hasOwnProperty('v') ? data[`I${dayIndex + i}`].v : 'rien à signaler'
      indivStats = {
        ...firstStats,
        comment: capiNote
      }
    }
    stats.push(indivStats)
    i++
  }
  return stats
}
// 9. PARSESHIFTS _________________________________________ */
/**
 * <b>DESCR:</b><br>
...
export { parser, dayPicker, readCol, readRow, findDay, weeklyStats, findNames, parseShift }